                ...
                if(item.link == path){
                    return (
                        <div key={item.title} onLoad={() => {
							...
							setPlaylistIndex(ARTISTS.indexOf(item))
						}}>
							...
							<div className={styles.PlaylistIcons}>
								<button
									onClick={() => props.changeTrack([ARTISTS.indexOf(item), 0])} 
								>
							...
							<div className={styles.PlaylistSongs}>
								{item.playlistData.map((song) => {
									return (
										<button 
											key={song.index} 
											onClick={() => props.changeTrack([ARTISTS.indexOf(item), item.playlistData.indexOf(song)])} 
...
export default connect(mapStateToProps, { changeTrack })(ArtistPage);