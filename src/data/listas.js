export let LISTAS = [

	/* 0 LIST Room Edition */
	{
		index: "0",
		type: "playlist",
		title: "Room Edition",
		link: "room-edition",
		playlistBg: "rgb(255, 193, 130)",
		imgUrl: "./image/set/room-edition/room-edition.jpg",
		hoverColor: "rgb(255, 193, 130)",
		artist: "Mixtape",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "https://instagram.com/ali.bhtty/",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistImages: [
			"./image/listas/room-edition/image1.jpg"
		  ],
		/* playlistImages: [
			"./image/listas/room-edition/image1.jpg",
			"./image/listas/room-edition/image2.jpg",
			"./image/listas/room-edition/image3.jpg"
		  ], */
		playlistData: [
			{
				index: "1",
				songName: "NOISE CONTROL",
				songimg: "./image/listas/room-edition/noise-control.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/scl/fi/z06q1ek561mm3tfpe53s8/NOISE-CONTROL-AliBhtty-Room-Edition.mp3?rlkey=abvpbmzpfvp84adjqz8crb6yt&dl=1",
				trackTime: "20:43",
			},
		],
	},

	/* 01 LIST Radioshow  🅰🅱🅲🅳🅴🅵🅶🅷🅸🅵🅺🅻🅼🅽🅾🅿🆀🆁🆂🆃🆄🆅🆆🆇🆈🆉 */
	{
		index: "1",
		type: "playlist",
		title: "Radioshow",
		link: "radioshow",
		playlistBg: "rgb(0, 179, 255)",
		imgUrl:	"./image/set/selection.jpg",
		hoverColor: "rgb(0, 179, 255)",
		artist: "Ali Bhtty - Bass music",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "https://instagram.com/ali.bhtty/",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistImages: [
			"./image/listas/room-edition/image1.jpg"
		  ],
		playlistData: [
			{
				index: "1",
				songName: "Ep.015 House Sauce",
				songimg: "./image/set/radioshow/015.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/s/qnogflgjdoyvc77/Ep.%20015%20House%20Sause%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.015-House-Sause-Ali-Bhtty.m4a */
				trackTime: "16:00",
			},
			{
				index: "2",
				songName: "Ep.014 BCN City Vibes",
				songimg: "./image/set/radioshow/014.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/s/azpd0pjc7ozif0n/Ep.%20014%20BCN%20City%20Vibes%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.014-BCN-City-Vibes-Ali-Bhtty.m4a */
				trackTime: "16:31",
			},
			{
				index: "3",
				songName: "Ep.013 Cy-BORED",
				songimg:
					"./image/set/radioshow/013.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://dl.dropbox.com/s/iw7it2twnpkxsm7/Ep.%20013%20Cy-bored%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.013-Cy-bored-Ali-Bhtty.m4a */
				trackTime: "14:55",
			},
			{
				index: "4",
				songName: "Ep.012 Turn It Up",
				songimg: "./image/set/radioshow/012.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/s/v36xlhw8ichvr4l/Ep.%20012%20Turn%20It%20Up%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.012-Turn-It-Up-Ali-Bhtty.m4a */
				trackTime: "16:34",
			},
			{
				index: "5",
				songName: "Ep.011 Blackouts",
				songimg:
					"./image/set/radioshow/011.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://dl.dropbox.com/s/bjffl4j12uqng34/Ep.%20011%20Blackouts%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.011-Blackouts-Ali-Bhtty.m4a */
				trackTime: "19:09",
			},
			{
				index: "6",
				songName: "Ep.010 Wild Therapy",
				songimg:
					"./image/set/radioshow/010.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://dl.dropbox.com/s/ll7ex466kzwpt77/Ep.%20010%20Wild%20Therapy%20-%20Ali%20Bhtty.m4a",  /* https://changarrito.es/assets/mus/ab/Ep.010-Wild-Therapy-Ali-Bhtty.m4a */
				trackTime: "32:03",
			},
			{
				index: "7",
				songName: "Ep.009 Hard EDM & Rock",
				songimg:
					"./image/set/radioshow/009.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.009-Hard-EDM&Rock-Ali-Bhtty.m4a",
				trackTime: "34:41",
			},
			{
				index: "8",
				songName: "Ep.008 Reggaeton 2021",
				songimg:
					"./image/set/radioshow/008.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.008-Reggaeton-2021-Ali-Bhtty.m4a",
				trackTime: "32:59",
			},
			{
				index: "9",
				songName: "🅴 Ep.007 Altering Reality",
				songimg:
					"./image/set/radioshow/007.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://dl.dropbox.com/s/tp5aon8i5g91lwp/Ep.%20007%20Altering%20Reality%20-%20Ali%20Bhtty.m4a", /*  https://changarrito.es/assets/mus/ab/Ep.007-Altering-Reality-Ali-Bhtty.m4a */
				trackTime: "39:31",
			},
			{
				index: "10",
				songName: "Ep.006 YHLQMDLG Sesion",
				songimg:
					"./image/set/radioshow/006.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://dl.dropbox.com/s/4f1he8ymqai22xf/Ep.%20006%20YHLQMDLG%20Bad%20Bunny%20-%20Ali%20Bhtty.m4a", /* https://changarrito.es/assets/mus/ab/Ep.006-YHLQMDLG-Bad-Bunny-Ali-Bhtty.m4a */
				trackTime: "34:31",
			},
			{
				index: "11",
				songName: "Ep.005 Salsatón",
				songimg:
					"./image/set/radioshow/005.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.005-Salsaton-Ali-Bhtty.m4a",
				trackTime: "34:41",
			},
			{
				index: "12",
				songName: "Ep.004 ElectroPOP",
				songimg:
					"./image/set/radioshow/004.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.004-ElectroPOP-Ali-Bhtty.m4a",
				trackTime: "32:59",
			},
			{
				index: "13",
				songName: "Ep.003 Progressive",
				songimg:
					"./image/set/radioshow/003.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.003-Progressive-Ali-Bhtty.m4a",
				trackTime: "34:41",
			},
			{
				index: "14",
				songName: "🅴 Ep.002 Reggaeton Old School",
				songimg:
					"./image/set/radioshow/002.jpg",
				songArtist: "Ali Bhtty",
				link:
					"https://changarrito.es/assets/mus/ab/Ep.002-Reggaeton-Old-School-Ali-Bhtty.m4a",
				trackTime: "32:59",
			},
			{
				index: "15",
				songName: "Ep.001 Dirty House",
				songimg: "./image/set/radioshow/001.jpg",
				songArtist: "Ali Bhtty",
				link: "https://changarrito.es/assets/mus/ab/Ep.001-Dirty-House-Ali-Bhtty.m4a",
				trackTime: "34:41",
			},
			{
				index: "16",
				songName: "🅴 Ep.000 Bounce Drop",
				songimg: "./image/set/radioshow/000.jpg",
				songArtist: "Ali Bhtty",
				link: "https://changarrito.es/assets/mus/ab/Ep.000-Bounce-Drop-Ali-Bhtty.m4a",
				trackTime: "32:59",
			},
		],
	},

	/* 02 LIST Collection */
	{
		index: "2",
		type: "playlist",
		title: "Mixtape Collection",
		link: "colección",
		playlistBg: "rgb(255, 94, 77)",
		imgUrl: "./image/set/alibhtty.jpg",
		hoverColor: "rgb(255, 94, 77)",
		artist: "Electronic 4ever – Bass music",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "Follow",
		artistDownload: "Descargar",
		playlistBg: "rgb(255, 94, 77)",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Turn On The Lights",
				songimg: "./image/disco/turn-on.jpg",
				songArtist: "Fred Again, Knock2",
				link: "https://dl.dropbox.com/s/mje86h2ubhxk6e0/Fred%20again%20-%20Turn%20On%20The%20Lights%20again%20%28Knock2%20Remix%29.mp3?dl=1",
				trackTime: "3:27",
			},
			{
				index: "2",
				songName: "Nowhere (Odd Mob Remix)",
				songimg: "./image/disco/nowhere.jpg",
				songArtist: "Otosan",
				link: "https://dl.dropbox.com/s/qfbat76durimxuf/Otosan%20-%20Nowhere%20%28Odd%20Mob%20Remix%29.mp3?dl=1",
				trackTime: "4:25",
			},
			{
				index: "3",
				songName: "Right Now (Ali Bhtty Remix)",
				songimg: "./image/disco/right-now.jpg",
				songArtist: "Ali Bhtty ft. Wxlfgvng!",
				link: "https://dl.dropbox.com/s/rd2tfbwmr5dc1iv/Right%20Now%20-%20Wxlfgvng%20%28Ali%20Bhtty%20Remix%29.mp3",
				trackTime: "3:15",
			},
		],
	},

	/* 03 LIST Top Spain */
	{
		index: "3",
		type: "playlist",
		title: "Top España",
		link: "top-spain",
		playlistBg: "rgb(255, 94, 77)",
		imgUrl: "./image/listas/top-spain/top-spain.jpg",
		hoverColor: "rgb(255, 94, 77)",
		artist: "Mixtape",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "Collection",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Supernova",
				songimg: "./image/listas/top-spain/supernova.jpg",
				songArtist: "Saiko",
				link: "https://dl.dropbox.com/s/1ylcet7f4j8rnnu/01%20-%20Saiko%20-%20Supernova.mp3",
				trackTime: "3:10",
			},
			{
				index: "2",
				songName: "EL TONTO",
				songimg: "./image/listas/top-spain/eltonto.jpg",
				songArtist: "Lola Indigo, Quevedo",
				link: "https://dl.dropbox.com/s/r8wy0gfgssuocbg/02%20-%20Lola%20Indigo%20-%20EL%20TONTO.mp3",
				trackTime: "3:07",
			},
			{
				index: "3",
				songName: "BESO",
				songimg: "./image/listas/top-spain/rr.jpg",
				songArtist: "ROSALÍA, Rauw Alejandro",
				link: "https://dl.dropbox.com/s/u5aayd0vfql75c2/09%20-%20ROSAL%C3%8DA%20-%20BESO.mp3",
				trackTime: "3:14",
			},
			{
				index: "4",
				songName: "El Merengue",
				songimg: "./image/listas/top-spain/elmerengue.jpg",
				songArtist: "Marshmello, Manuel Turizo",
				link: "https://dl.dropbox.com/s/2qwxgs35w5k9l1z/05%20-%20Marshmello%20-%20El%20Merengue.mp3",
				trackTime: "3:09",
			},
			{
				index: "5",
				songName: "Mami Chula",
				songimg: "./image/listas/top-spain/mamichula.jpg",
				songArtist: "Jhayco, Quevedo",
				link: "https://dl.dropbox.com/s/zjxhswzy6ck0dg4/04%20-%20Jhayco%20-%20Mami%20Chula.mp3",
				trackTime: "3:40",
			},
			{
				index: "6",
				songName: "Classy 101",
				songimg: "./image/listas/top-spain/classy101.jpg",
				songArtist: "Feid, Young Miko",
				link: "https://dl.dropbox.com/s/u0g4193fpm4glk8/22%20-%20Feid%20-%20Classy%20101.mp3",
				trackTime: "3:15",
			},
			{
				index: "7",
				songName: "Nochentera",
				songimg: "./image/listas/top-spain/nochentera.jpg",
				songArtist: "Vicco",
				link: "https://dl.dropbox.com/s/dt2i8yalciizreq/15%20-%20Vicco%20-%20Nochentera.mp3",
				trackTime: "3:22",
			},
			{
				index: "8",
				songName: "TQG",
				songimg: "./image/listas/top-spain/tqg.jpg",
				songArtist: "KAROL G, Shakira",
				link: "https://dl.dropbox.com/s/f4lyxqvnd9l3hjo/KAROL%20G%20-%20TQG.mp3",
				trackTime: "3:17",
			},
			{
				index: "9",
				songName: "Clavaito",
				songimg: "./image/listas/top-spain/clavaito.jpg",
				songArtist: "Chanel, Abraham Mateo",
				link: "https://dl.dropbox.com/s/q2zsfkcqm4dq7q1/Chanel%20-%20Clavaito.mp3",
				trackTime: "2:42",
			},
			{
				index: "10",
				songName: "Yandel 150",
				songimg: "./image/listas/top-spain/yandel150.jpg",
				songArtist: "Yandel, Feid",
				link: "https://dl.dropbox.com/s/03d5tfgoo88np7r/04%20-%20Yandel%20-%20Yandel%20150.mp3",
				trackTime: "3:36",
			},
		],
	},

	 /* 00 LIST Club Urbano */
	 {
		index: "4",
		type: "playlist",
		title: "Club Urbano",
		link: "urbano",
		playlistBg: "rgb(95, 74, 255)",
		imgUrl:	"./image/set/urbano.jpg",
		hoverColor: "rgb(95, 74, 255)",
		artist: "Ali Bhtty – Urban music",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Club Urbano 01",
				songimg: "./image/set/urbano/urbano01.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/scl/fi/hxmbva1tpo9ecwv2iqzdw/Club-Urbano-01.m4a?rlkey=n4t40ot0gtqo80u9ow9e6srde&dl=1", /* https://dl.dropbox.com/s/qnogflgjdoyvc77/Ep.%20015%20House%20Sause%20-%20Ali%20Bhtty.m4a */
				trackTime: "23:37",
			},
		],
	},

	/* 05 LIST Knock2 */
	{
		index: "5",
		type: "playlist",
		title: "Trap in comming",
		link: "favoritos",
		playlistBg: "#fc7272",
		imgUrl:	"./image/set/nov2023/trap5.jpg",
		hoverColor: "#fc7272",
		artist: "Mixtape",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "Future Trap & more",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Lollipop",
				songimg: "./image/listas/hits-urbano/01.jpg",
				songArtist: "Darell",
				link: "https://dl.dropbox.com/scl/fi/f3k1aw9qvgokmt9vkwu0d/Darell-Lollipop.mp3?rlkey=w1ch5xvoeg5ltrzzerr7vzslj&dl=1",
				trackTime: "3:20",
			},
			{
				index: "2",
				songName: "PERRO NEGRO",
				songimg: "./image/listas/hits-urbano/02.jpg",
				songArtist: "Bad Bunny",
				link: "https://dl.dropbox.com/scl/fi/3lzm8uzrv542geql2ksel/Bad-Bunny-PERRO-NEGRO.mp3?rlkey=rui03xf0gpxj3xkvl1e1gngzs&dl=1",
				trackTime: "2:43",
			},
			{
				index: "3",
				songName: "Corazón Roto pt. 3",
				songimg: "./image/listas/hits-urbano/03.jpg",
				songArtist: "Brray",
				link: "https://dl.dropbox.com/scl/fi/dxbxtjidy7mq36n8ocmt7/Brray-Coraz-n-Roto-pt.-3.mp3?rlkey=lpc4veabl000mor2fmnkuihqs&dl=1",
				trackTime: "5:48",
			},
			{
				index: "4",
				songName: "Cabaña",
				songimg: "./image/listas/hits-urbano/04.jpg",
				songArtist: "El Jordan 23",
				link: "https://dl.dropbox.com/scl/fi/tb0cxiys2lbf0k4v5trj4/El-Jordan-23-Caba-a.mp3?rlkey=fwkkf476qkic1536eos0uu52j&dl=1",
				trackTime: "2:59",
			},
			{
				index: "5",
				songName: "Labios Mordidos",
				songimg: "./image/listas/hits-urbano/05.jpg",
				songArtist: "Kali Uchis",
				link: "https://dl.dropbox.com/scl/fi/ow29ap155x3yh5ihuben3/Kali-Uchis-Labios-Mordidos.mp3?rlkey=rr3qia9e9e3sfpafxhtjmra4s&dl=1",
				trackTime: "3:15",
			},
			{
				index: "6",
				songName: "UN PREVIEW",
				songimg: "./image/listas/hits-urbano/06.jpg",
				songArtist: "Bad Bunny",
				link: "https://dl.dropbox.com/scl/fi/5ichosuh7yel4mx0hfbh0/Bad-Bunny-UN-PREVIEW.mp3?rlkey=sskvmhhx00dt495fl30tcin8l&dl=1",
				trackTime: "2:46",
			},
			{
				index: "7",
				songName: "COSMETICOS",
				songimg: "./image/listas/hits-urbano/07.jpg",
				songArtist: "Myke Towers",
				link: "https://dl.dropbox.com/scl/fi/t23ic6hbwow23wi8dglra/Myke-Towers-COSMETICOS.mp3?rlkey=vz8pcnbdy47xe77o5d9xblmfx&dl=1",
				trackTime: "2:22",
			},
			{
				index: "8",
				songName: "SANDUNGA",
				songimg: "./image/listas/hits-urbano/08.jpg",
				songArtist: "Don Omar",
				link: "https://dl.dropbox.com/scl/fi/kjay3ctytil34xm7w5w3j/Don-Omar-SANDUNGA.mp3?rlkey=6hln05j1iw60awrubfqh9pako&dl=1",
				trackTime: "4:02",
			},
			{
				index: "9",
				songName: "LA VUELTA",
				songimg: "./image/listas/hits-urbano/09.jpg",
				songArtist: "Feid",
				link: "https://dl.dropbox.com/scl/fi/hna0zj3vhl1tj0xz152kl/Feid-LA-VUELTA.mp3?rlkey=b7d9ka17k231cxmf8i0xd3ybe&dl=1",
				trackTime: "2:02",
			},
			{
				index: "10",
				songName: "LA INOCENTE",
				songimg: "./image/listas/hits-urbano/10.jpg",
				songArtist: "Mora",
				link: "https://dl.dropbox.com/scl/fi/xvzi0aip1vllti7ulndxa/Mora-LA-INOCENTE.mp3?rlkey=946r6dqvn8if9y5hnmc2snmqh&dl=1",
				trackTime: "3:22",
			},
			/* {
				index: "11",
				songName: "Turn On The Lights",
				songimg: "./image/listas/hits-urbano/11.jpg",
				songArtist: "Fred Again, Knock2",
				link: "https://dl.dropbox.com/s/mje86h2ubhxk6e0/Fred%20again%20-%20Turn%20On%20The%20Lights%20again%20%28Knock2%20Remix%29.mp3",
				trackTime: "3:27",
			},
			{
				index: "12",
				songName: "Calling (Ali Bhtty Remix)",
				songimg: "./image/listas/hits-urbano/12.jpg",
				songArtist: "Jalen ft. Ali Bhtty",
				link: "https://dl.dropbox.com/s/67upkqw2hpgdz5w/Calling%20-%20Jalen%20%28Ali%20Bhtty%20Remix%29.mp3",
				trackTime: "4:12",
			},
			{
				index: "13",
				songName: "Turn On The Lights",
				songimg: "./image/listas/hits-urbano/13.jpg",
				songArtist: "Fred Again, Knock2",
				link: "https://dl.dropbox.com/s/mje86h2ubhxk6e0/Fred%20again%20-%20Turn%20On%20The%20Lights%20again%20%28Knock2%20Remix%29.mp3",
				trackTime: "3:27",
			},
			{
				index: "14",
				songName: "Calling (Ali Bhtty Remix)",
				songimg: "./image/listas/hits-urbano/14.jpg",
				songArtist: "Jalen ft. Ali Bhtty",
				link: "https://dl.dropbox.com/s/67upkqw2hpgdz5w/Calling%20-%20Jalen%20%28Ali%20Bhtty%20Remix%29.mp3",
				trackTime: "4:12",
			},
			{
				index: "15",
				songName: "Turn On The Lights",
				songimg: "./image/listas/hits-urbano/15.jpg",
				songArtist: "Fred Again, Knock2",
				link: "https://dl.dropbox.com/s/mje86h2ubhxk6e0/Fred%20again%20-%20Turn%20On%20The%20Lights%20again%20%28Knock2%20Remix%29.mp3",
				trackTime: "3:27",
			},
			{
				index: "16",
				songName: "Calling (Ali Bhtty Remix)",
				songimg: "./image/listas/hits-urbano/16.jpg",
				songArtist: "Jalen ft. Ali Bhtty",
				link: "https://dl.dropbox.com/s/67upkqw2hpgdz5w/Calling%20-%20Jalen%20%28Ali%20Bhtty%20Remix%29.mp3",
				trackTime: "4:12",
			}, */
		],
	},

	/* 06 LIST Mood2 - demotapes */
	{
		index: "6",
		type: "playlist",
		title: "Mood",
		link: "mood",
		playlistBg: "rgb(255, 171, 38)",
		imgUrl:	"./image/set/demotapes-bg.jpg", /* https://dailymix-images.scdn.co/v2/img/04cd9bccf59e8061f13e779d1001975a60fb837a/2/tr/default */
		hoverColor: "rgb(255, 171, 38)",
		artist:	"En tu peak.",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "demotapes with friends mix",
				songimg: "./image/set/demotapes1x1.jpg",
				songArtist: "demotapes",
				link: "https://dl.dropbox.com/scl/fi/8qku4fxr8leiunvvwaxib/demotapes-with-friends-mix.m4a?rlkey=d9ho6sfoyy9ejq8apd0qaheo8&dl=1",
				trackTime: "19:56",
			},
		],
	},

	/* 07 LIST Remixes */
	{
		index: "7",
		type: "playlist",
		title: "Remixes",
		link: "remixes",
		playlistBg: "rgb(249, 255, 168)",
		imgUrl:	"./image/set/remixes.jpg",
		hoverColor: "rgb(249, 255, 168)",
		artist:	"DJ's de Mixtape",
		artistImgUrl: "./image/djs/mixtape/mixtape.png",
		artistBio: "All Remixes",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Turn On The Lights",
				songimg: "./image/disco/turn-on.jpg",
				songArtist: "Fred Again, Knock2",
				link: "https://dl.dropbox.com/s/mje86h2ubhxk6e0/Fred%20again%20-%20Turn%20On%20The%20Lights%20again%20%28Knock2%20Remix%29.mp3?dl=1",
				trackTime: "3:27",
			},
			{
				index: "2",
				songName: "Embrace",
				songimg: "./image/disco/mith-1x1.jpg",
				songArtist: "Ali Bhtty",
				link: "https://dl.dropbox.com/s/vy8kcd79brun966/Mith%20-%20Ali%20Bhtty.m4a",
				trackTime: "2:48",
			},
			{
				index: "3",
				songName: "Calling",
				songimg: "./image/disco/calling.jpg",
				songArtist: "Jalen, Ali Bhtty",
				link: "https://dl.dropbox.com/s/e4y32w713qjcekj/1-01%20Calling%202%20%28feat.%20Ali%20Bhtty%29.m4a",
				trackTime: "4:12",
			},
		],
	},

	/* 8 ALBUM Calling */
	{
		index: "8",
		type: "album",
		title: "Calling",
		link: "calling",
		playlistBg: "rgb(255, 102, 0)",
		imgUrl:	"./image/set/calling.jpg",
		hoverColor: "rgb(255, 102, 0)",
		artist: "Jalen ft. Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "https://instagram.com/ali.bhtty/",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Calling (Ali Bhtty Remix)",
				songimg: "./image/disco/calling.jpg",
				songArtist: "Jalen ft. Ali Bhtty",
				link: "https://dl.dropbox.com/s/e4y32w713qjcekj/1-01%20Calling%202%20%28feat.%20Ali%20Bhtty%29.m4a",
				trackTime: "4:12",
			},
			{
				index: "2",
				songName: "Calling",
				songimg: "./image/disco/calling-o.jpg",
				songArtist: "Jalen",
				link: "https://dl.dropbox.com/s/p6c083jsb53ow3k/1-01%20CALLING.m4a",
				trackTime: "2:55",
			},
		],
	},

	/* 9 ALBUM Cage Script */
	{
		index: "9",
		type: "album",
		title: "Cage Script",
		link: "cage-script",
		playlistBg: "rgb(136, 157, 207)",
		imgUrl:	"./image/set/cage-script.jpg",
		hoverColor: "rgb(136, 157, 207)",
		artist: "k?d – Future EDM",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Be Mine",
				songimg: "./image/disco/cage-script.jpg",
				songArtist: "K?d",
				link: "https://dl.dropbox.com/s/bsxoax5xyoqcnfj/09%20-%20BE%20MINE.mp3",
				trackTime: "4:57",
			},
			{
				index: "2",
				songName: "Three in the morning",
				songimg: "./image/disco/cage-script.jpg",
				songArtist: "K?d",
				link: "https://dl.dropbox.com/s/7okhtbw1ach604z/04%20-%20THREE%20IN%20THE%20MORNING.mp3",
				trackTime: "2:51",
			},
			{
				index: "3",
				songName: "Home Player",
				songimg: "./image/disco/cage-script.jpg",
				songArtist: "K?d",
				link: "https://dl.dropbox.com/s/bxdbj90s18qoz0g/10%20-%20HOME%20PRAYER.mp3",
				trackTime: "3:41",
			},
		],
	},

	/* 10 PODCAST Embrace */
	{
		index: "10",
		type: "podcast",
		title: "Embrace",
		link: "chillout",
		playlistBg: "rgb(96, 162, 191)",
		imgUrl:	"./image/podcast/serie/chillout/embrace.jpg",
		hoverColor: "rgb(96, 162, 191)",
		artist: "Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Chill out",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "Embrace",
				songimg: "./image/podcast/serie/chillout/embrace.jpg",
				songArtist: "Ali Bhtty",
				link: "https://changarrito.es/assets/mus/Mith-Ali-Bhtty.m4a",
				trackTime: "2:48",
			},
		],
	},

	/* 11 LIST */
	{
		index: "11",
		type: "podcast",
		title: "Desfoguez",
		link: "desfoguez",
		playlistBg: "rgb(42, 77, 48)",
		imgUrl:	"./image/podcast/serie/desfoguez/quien-soy.jpg",
		hoverColor: "rgb(42, 77, 48)",
		artist: "",
		artistImgUrl: "./image/podcast/caster/desfoguez.jpg",
		artistBio: "https://www.youtube.com/channel/UCd5wDYOz1eMnt0i6ZXR1TDg",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			{
				index: "1",
				songName: "¿Y si nunca supieras quien eres?",
				songimg: "./image/podcast/serie/desfoguez/quien-soy.jpg",
				songArtist: "Desfoguez",
				link: "https://dl.dropbox.com/scl/fi/74hfto6w85urng4z0a5h3/Y-SI-NUNCA-SUPIERAS-QUIEN-ERES.mp3?rlkey=xua6lv5j32i6dr1vxdo7cdx74&dl=1",
				trackTime: "8:59",
			},
			{
				index: "2",
				songName: "Estancado",
				songimg: "./image/podcast/serie/desfoguez/estancado.jpg",
				songArtist: "Desfoguez",
				link: "https://dl.dropbox.com/scl/fi/1yn98403ujaxjs1428xti/ESTANCADO-Lo-que-deber-as-considerar-y-no-no-es-un-video-motivacional-de-mierda.mp3?rlkey=pvr4xu8c3uuirjowsvulz4kt2&dl=1",
				trackTime: "7:39",
			},
		],
	},

	/* 12 LIST */
	{
		index: "12",
		type: "playlist",
		title: " demo2 ",
		link: " ",
		playlistBg: "rgb(255, 102, 0)",
		imgUrl:	" ",
		hoverColor: "rgb(255, 102, 0)",
		artist: "Jalen ft. Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			/* {
				index: "1",
				songName: " ",
				songimg: " ",
				songArtist: "Jalen ft. Ali Bhtty",
				link: " ",
				trackTime: " ",
			}, */
		],
	},

	/* 13 LIST */
	{
		index: "13",
		type: "playlist",
		title: " demo3 ",
		link: " ",
		playlistBg: "rgb(255, 102, 0)",
		imgUrl:	" ",
		hoverColor: "rgb(255, 102, 0)",
		artist: "Jalen ft. Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			/* {
				index: "1",
				songName: " ",
				songimg: " ",
				songArtist: "Jalen ft. Ali Bhtty",
				link: " ",
				trackTime: " ",
			}, */
		],
	},

	/* 14 LIST */
	{
		index: "14",
		type: "playlist",
		title: " demo4 ",
		link: " ",
		playlistBg: "rgb(255, 102, 0)",
		imgUrl:	" ",
		hoverColor: "rgb(255, 102, 0)",
		artist: "Jalen ft. Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			/* {
				index: "1",
				songName: " ",
				songimg: " ",
				songArtist: "Jalen ft. Ali Bhtty",
				link: " ",
				trackTime: " ",
			}, */
		],
	},

	/* 15 LIST */
	{
		index: "15",
		type: "playlist",
		title: " demo5 ",
		link: " ",
		playlistBg: "rgb(255, 102, 0)",
		imgUrl:	" ",
		hoverColor: "rgb(255, 102, 0)",
		artist: "Jalen ft. Ali Bhtty",
		artistImgUrl: "./image/djs/alibhtty/alibhtty.jpg",
		artistBio: "Follow",
		artistDownload: "Descargar",
		bannerUno: "",
		downloadData: [],
		playlistData: [
			/* {
				index: "1",
				songName: " ",
				songimg: " ",
				songArtist: "Jalen ft. Ali Bhtty",
				link: " ",
				trackTime: " ",
			}, */
		],
	},
]